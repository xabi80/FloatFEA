...
# Revision 13 — the counter is an injected defect again

**2026-09-06.** Two commits since the twelfth verdict: `88b2d5e` (plan, re-locked)
and `a458662` (step).
...
corpus: 90 entries executed
        branches: inadmissible 5, measured 69, refused 6, unparseable 10
```

**Green on the reviewer's twelfth-round corpus**, including the seven entries that
were red at `121a35a` — and none of them by moving a number, skipping, `xfail`ing
or deleting an entry. Two of the seven (`aniso_zero_I_y_must_refuse`,
`aniso_negative_I_y_must_refuse`) go green by **raising**, which is what they ask
for.

---

## 0. A false claim in the step commit's own message, corrected here

`a458662` says `grep -n "2.5x\|BAND_COUNTER" docs/milestones/F2.md` **is empty**.
It is not: it returns two lines, 480 and 546. Both are the *withdrawal* text —
"`PATCH_TEST_SENSITIVITY_BAND_COUNTER` … is removed" — so R106's stale
**claim** is gone and the symbol survives only where the plan says it was
removed. The check that is actually true, and that I should have run:

```
$ grep -n "detects a 2.5x\|BAND_COUNTER = 3.0\|= 3.0\`" docs/milestones/F2.md
(nothing)
```

I wrote the grep from memory of what I had deleted instead of running the one I
quoted. That is BF0's failure in the commit that cites BF0, and it is recorded
first because a report that buries its own bad claim is worth less than the claim
was worth.

## 1. What was actually wrong, and it was the form

**Every "undetectable" corpus entry since R94 was an artefact of the assertion's
shape.** At `L/r_min = 2885` the wrong-DOF defect responds at `3.5e-06` against a
ceiling of `5e-15` — **nine orders of detection** — and was reported as a wall
because the assertion compared it to `3.5e-05`. The counter had drifted from *the
injected defect whose presence must make the gate red* into *a floor on the
response*, under three successive numbers: `1.0e-7`, `1.0e-13`, `3.5e-05`.

**And "structural defects do not fall with slenderness" is withdrawn.** The
residual normalises by the largest stiffness — the axial one — so any defect in a
bending block falls as `1/λ_weak²`. Both structural defects fall at exponent
`−1.99`, the small defect's own. It stops mattering once the comparison is to the
ceiling.

**The alternative was declined in the plan.** Normalising by the smallest bending
stiffness makes the response slenderness-free and raises the round-off floor as
`λ²` — the same physics relabelled, and a floor-aware ceiling again, which a STOP
already closed.

## 2. BO1, measured before anything was written, regenerated by the shipped runner

```
MARGINS  response / ceiling, reported and asserted against no constant
  dropped_flip        minimum  5.150e+09x   at flip_counter_edge_thickwall (L/r_min 2332)
  wrong_dof_index     minimum  5.227e+09x   at lam1058_axis_counter_red    (L/r_min 1058)
  one_element_scaled  minimum  8.701e+06x   at slender_axis_L_r_189        (L/r_min  558)

CEILING  5.000e-15    worst clean 4.4363e-16 (2.00 eps, band_edge_isotropic_bracing) = 0.0887x
```

**Zero entries fail to redden**, at any slenderness the corpus reaches.

## 3. Two defects recorded as NOT corpus-wide, with the mechanism

**The transposed transform is a no-op on an axis-aligned member.** `R.T` differs
from `R` only when `R` is not symmetric, and an axis-aligned member with no roll
has `R = I`. Measured: `|R − R.T| = 0` **exactly** on 8 corpus entries, where the
response equals the clean value; on 2 further near-axis entries the injected
difference leaves the balance below the ceiling. It stays the **skew**
counter-case it already is, and its docstring now says where it is vacuous. Same
species as `I_y ↔ I_z` (R53) — the defect is absent, not undetected.

**The dropped shear parameter** is undetectable on 22 of 63 entries because
`Φ ∝ 1/λ²` makes the *defect itself* vanish on a slender member: there is nothing
left to corrupt. That is V2.2's defect, on a stubby member, and V2.2 owns it
(BO3).

## 4. The curve is a diagnostic (BO2), and the scatter argues for it

```
CURVE  weakest-state response = 3.505e-08 * (L/r_min)^(-1.968)
       scatter 0.763x .. 2.496x     DIAGNOSTIC, asserted nowhere
```

The exponent is the mechanism's `−2`. The scatter is **3.3× wide** across the
90-entry corpus — wider than the 2.2× the band was fitted to at 63 entries, which
is the same finding R107 made from the other side. A band on it was already at
`1.01×` of margin over the reviewer's envelope with its own counter failing
downward; dropping it loses nothing, because the red-on-defect check *is* the
sensitivity guard.

**R108's per-state claim is back, for the one defect that supports it.** A
structural defect corrupts one block, so states with no content in it are blind by
construction — R1's plane-confinement property, not a weakness. A scaled *element*
touches every block, so every state must respond:
`test_EVERY_STATE_detects_the_small_defect`, worst **`1.867×`** the ceiling at
`L/r_min = 2332`. Thin, and it is the honest number.

## 5. BO4

| item | what changed |
|---|---|
| R106 | plan and code agree — see §0 for the check that is true and the one I mis-quoted |
| R109 | **moot** — `SENSITIVITY_SCALE` no longer decides a pass; it is gone |
| R110 | the phantom `SENSITIVITY_CURVE` pointer went with the block |
| R111 | **moot** under BO2 |
| R112 | `admissibility.py`'s withdrawn framing rewritten. The justification that a counter is "recorded at the most slender configuration" went with the form it belonged to; what stays is that every defect's response falls as `1/λ_weak²`, which makes this a useful axis to *report* against and gates nothing |
| R114 | `I_y_over_I_z ≤ 0` raises. It built **zero** bending stiffness about one axis and `−1.0` built a **negative** one, silently, because `object.__setattr__` writes past `Section.__post_init__` — the only guard |

## 6. Carried — every open item, by number

| item | status |
|---|---|
| R105 | **answered** — the form, not a domain. Zero entries fail to redden |
| R106 | **answered**, with the correction in §0 |
| R107 | **answered by removal** — the band is gone; its 1.01× margin is recorded as the reason |
| R108 | **answered** — the per-state check is back, worst `1.867×` |
| R109, R111 | **moot** under BO2, recorded |
| R110, R112, R114 | **closed** |
| R113 | **open** — not addressed this round |
| R95 | **open** — the `expect=raise` branch uses `pytest.raises(ValueError)` with no `match` |
| R97 | **open** — the plan says "never asserted at a constant" while the gate asserts `res_err <= RESULTANT_EXACTNESS` on the solved field; the resultant channel is a different quantity and the plan does not say so |
| R98 | **open** — `INADMISSIBLE` assigned and read nowhere |
| R100 | **open** — three tables in `tolerances.py` with no generator; the mechanism is step 4a's |
...
| R103 | **open** — the BM0 table's mislabelled row, in revision 11 |
| R63 | **open** — `MATRIX_SYMMETRY` and `ROUNDOFF_IDENTITY` widenable in silence |
| R65 | **withdrawn by the reviewer** |
| R76, R79, R80 | **open** |
| R6, R16, R25, R30, R31, R32, R33 (outside G2.2), R36, R50, R52, R62 | **open** — step 4a or later |
| R77, R78, R81–R94, R96, R99, R104 | **closed** in earlier rounds |

## 7. Where I was wrong inside this round

1. **The grep in the step commit's message**, §0. Quoted, not run.
2. **The first cut of the red-on-defect test deleted `_oob_with_defect`** along
   with the block above it. 141 tests failed with a `NameError` rather than a
   wrong number, which is the failure mode to want, and it took one run to find.
3. **The regenerated curve was first fitted to the worst state**, which gave an
   exponent of `+0.011` — the axial state does not fall with slenderness. The
   curve describes the *weakest* state; the margins describe the worst, because
   the gate fails if any state exceeds. Two different reductions, and mixing them
   produced a number that would have looked like a refutation of the mechanism.
