...
def test_sigma_allow_is_the_project_basis_not_a_safety_factor() -> None:
    ...
    """
    assert pytest.approx(213.0e6, rel=ROUNDOFF_IDENTITY) == sec.SIGMA_ALLOW_S355
    assert (
        sec.ALLOWABLE_FACTOR == 0.6
    )  # not-a-tolerance: the allowable FACTOR itself, an exact declared constant
    wrong = sec.FY_S355 / 1.5
    assert wrong / sec.SIGMA_ALLOW_S355 == pytest.approx(
        1.1111,
        abs=1e-4,
...
def test_kappa_matches_cowper() -> None:
    ...
    assert sec.kappa("thin_tube", 0.3) == pytest.approx(
        0.5305,
        abs=5e-4,
        # not-a-tolerance: reference pin, not a ceiling -- asserts a RECORDED measurement is
        # unchanged
    )
    assert sec.kappa("solid_circular", 0.3) == pytest.approx(
        0.8864,
        abs=5e-4,
        # not-a-tolerance: reference pin, not a ceiling -- asserts a RECORDED measurement is
        # unchanged
...
@pytest.mark.parametrize("fy, expected", [(235e6, 0.00400), (355e6, 0.00604), (460e6, 0.00783)])
def test_the_bound_scales_with_the_material_and_survives_all_of_them(
    ...
    assert sec.shear_geometric_bound(fy=fy) == pytest.approx(
        expected, abs=5e-5
    )  # not-a-tolerance: reference pin -- the recorded per-material bound
    assert (
        sec.shear_geometric_bound(fy=fy) < 0.01
    )  # not-a-tolerance: fixture property -- bounds an input, not a computed discrepancy
...
def test_kappa_is_distinguishable_from_the_simple_argument() -> None:
    ...
    simple = 0.5
    assert (
        abs(sec.kappa("thin_tube", 0.3) - simple) / simple
        > 0.05
...
def test_shear_modulus_is_derived_not_declared() -> None:
    ...
    assert (
        pytest.approx(sec.E_STEEL / (2 * (1 + sec.NU_STEEL)), rel=ROUNDOFF_IDENTITY) == sec.G_STEEL
    )
...
def test_tube_properties_are_exact_not_thin_walled(d: float, t: float) -> None:
    ...
    assert a_exact == pytest.approx(math.pi * (d**2 - (d - 2 * t) ** 2) / 4, rel=ROUNDOFF_IDENTITY)
    assert a_exact < a_thin  # thin-wall over-states area
    assert i_exact < i_thin
    assert (
        abs(i_exact - i_thin) / i_exact
        > 1e-3
        # not-a-tolerance: discrimination floor -- asserts a quantity is LARGE, not that an error is
        # small
    ), "difference too small to detect a swap"
