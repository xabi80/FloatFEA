...
def test_an_unseeded_arpack_start_would_NOT_be_reproducible() -> None:
    ...
    """
    np.random.seed(None)
    a = np.random.standard_normal(64)
    np.random.seed(None)
    b = np.random.standard_normal(64)
...
@pytest.mark.parametrize("member", ["threads", "sparse_permc_spec", "arpack_v0_seed",
                                    "pythonhashseed", "hypothesis_derandomize"])
