...
_GENERATED_MARK = re.compile(r"<!--\s*generated:\s*\S+\s*-->")
...
def _unsourced(lines: list[str]) -> list[tuple[str, str]]:
    ...
    # A GENERATED SECTION IS SOURCED BY ITS GENERATOR, EVERY LINE OF IT.
    # The section opens with `<!-- generated: scripts/... -->` and a
    # `Generated: python ...` line naming the invocation that reproduces it,
    # which is the whole of what this test asks for -- applied to the section
    # rather than to each line.
    #
    # It became reachable when a CI section first reported a FAILING run: the
    # failing-test names `gh` returns carry line numbers, as in
    # `...[R481-tests/test_report_carried.py:1402]`, and `1402` read as an
    # unsourced figure in prose. The number came out of `gh run view`; asking
    # the report to show a command for it would be asking it to source the
    # generator's own output by hand.
    #
    # WHAT THIS DOES NOT COVER, and it is covered elsewhere: prose a hand adds
    # INSIDE a generated span. `tests/test_report_carried.py` owns the
    # generated/hand-written split, and the reviewer's corpus carries
    # `a_false_narrative_paragraph_appended_inside_the_generated_0a_section`
    # as a shape the CI guards do not catch -- a `False` row there and an item
    # on the frozen 4a list.
    if any(_GENERATED_MARK.search(ln) for ln in prose):
        return out
    for line in prose:
