...
# Forms that are not figures. Checked against the CONTEXT around the match.
_EXEMPT_CONTEXT = (
...
def _unsourced(lines: list[str]) -> list[tuple[str, str]]:
    ...
    for line in prose:
        for m in _NUMBER.finditer(line):
            token = m.group(0)
            window = line[max(0, m.start() - 24) : m.end() + 12]
            if any(p.search(window) for p in _EXEMPT_CONTEXT):
                continue
            if token in sourced:
                continue
