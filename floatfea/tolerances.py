...
# twenty-eight frames showed what that measures: sixteen of them exceed its
...
# Reason for 1.0e-14: the response is linear in the defect over eight decades,
# and a diagonal stiffness resisting a rigid translation at this size takes the
# residual to about `5e-14`, which is well past the ceiling without sitting on
# its own detection edge. The sweep is in the step report, regenerated by rule
# and not tabulated here (BI3).
# Set: 2026-09-13, F2
RIGID_MODE_EXACTNESS_COUNTER_DEFECT: Final[float] = 1.0e-14

# CLASS: ACCURACY -- carries RIGID_MODE_GAP_COUNTER_DEFECT below.
# G2.1 / V1.1, THE COUNT (Q7). The spectrum is homogenised -- divided by
# `eps * max|lambda|`, the matrix's own round-off floor -- and the number of
# modes below the LARGEST gap in it is the number of rigid-body modes. This
# entry is the floor on that gap's ratio: how far apart the two groups have to
# be before the count is worth reading.
#
# WHY A GAP AND NOT A THRESHOLD. A threshold on an eigenvalue is a statement
# about the model's units and stiffness; the gap is a statement about the
# spectrum's own shape, and it is what a reader of a nullspace dimension
# actually relies on. Homogenising by the round-off floor is what makes it
# comparable between frames: the six zero modes sit BELOW that floor and the
# seventh is orders above it.
#
# Reason for 1e+06: the smallest gap over the reviewer's twenty-eight frames
# is about sixty times this, at the kilometre re-expression, which is the
# harshest conditioning in the file. Published by name in the step report.
# Set: 2026-09-13, F2
RIGID_MODE_GAP: Final[float] = 1.0e06

# COUNTER-CASE, INJECTED into the assembled matrix and run through the gate.
# Reason for 1.0e-07: a uniform elastic foundation of this size, as a fraction
# of the largest entry, lifts all six rigid modes together and closes the gap
# to about `9.7e+04` -- ten times past the floor -- while the count stays at
# six, which is the point: it degrades the gap rather than changing the answer.
# IT ALSO REDDENS THE RESIDUAL, necessarily, because anything that lifts a zero
# mode makes it carry energy. That is stated rather than hidden; what makes
# this the GAP's counter is that it is declared against the gap and sized on it.
# Set: 2026-09-13, F2
RIGID_MODE_GAP_COUNTER_DEFECT: Final[float] = 1.0e-07
