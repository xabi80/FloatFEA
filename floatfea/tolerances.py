...
# COUNTER-CASE -- and it is the FORMULATION-defect control, not a small-defect
# threshold (F2.md sec. 5b Q6, "the gate makes TWO claims").
# A defect that changes the element's STRUCTURE -- a dropped `flip`, a wrong DOF
# index, a transposed transform -- must leave an interior out-of-balance above
# this on EVERY corpus entry, with no exception and no domain.
#
# WHY THIS IS NOT THE 1e-6 THRESHOLD IT REPLACES. The residual measures a
# defect's contribution against the LARGEST stiffness in the matrix, so a defect
# in the weakest bending mode falls as the ratio of the two, `1/lambda_weak^2`.
# There is NO slenderness at which a fixed small defect stays visible; six review
# rounds were spent bounding a domain around a universal the physics forbids.
# Structural defects do not fall that way: they corrupt the block itself and stay
# `O(1)` relative to it.
#
# Reason for 3.5e-05: the smallest response over all 63 solved corpus entries and
# both injected structural defects, worst state per entry --
#
#     dropped flip          5.8789e-05   at L/r_min 1539
#     one wrong DOF index   3.5991e-05   at L/r_min 900
#
# -- so the counter sits just under the weaker of the two, and every entry in the
# corpus detects both by seven orders. The table is regenerated by the shipped
# corpus runner and printed in docs/reports/F2/step-4.md (BI3).
#
# `I_y <-> I_z` WAS PROPOSED AS A THIRD CONTROL AND IS REFUTED, recorded here
# because a control that cannot fail is worse than no control. Every shape
# `basis.kappa` admits forces `I_y == I_z`, so the swap is a no-op; on the
# synthetic anisotropic entries it measures 8.247e-17 against a clean 2.191e-16,
# i.e. BELOW the clean value. That is R53's blindness, and it is why the third
# structural control is the transposed transform.
# Set: 2026-09-03, F2. Quantity REPLACED 2026-09-06; redefined as the
# formulation-defect control 2026-09-06, F2.
PATCH_TEST_EXACTNESS_COUNTER: Final[float] = 3.5e-05


# CLASS: ACCURACY -- the gate's SECOND claim, and it is a BAND ON A CURVE rather
# than a threshold. Carries PATCH_TEST_SENSITIVITY_BAND_COUNTER below.
#
# G2.2 / V1.2 -- the interior out-of-balance produced by a 1e-6 relative stiffness
# error in ONE interior element is a function of the member's WEAK-AXIS
# slenderness, `response = SCALE * (L/r_min) ** EXPONENT`. The curve itself is a
# RECORDED MEASUREMENT and lives beside the code that consumes it, as
# `SENSITIVITY_CURVE` in `tests/verification/rung1/test_corpus_configurations.py`
# -- the same status as `DETECTION_THRESHOLD`. What lives HERE is the only number
# in it that decides a pass: the band.
#
# The band the measured response must sit inside, relative to the larger operand,
# asserted TWO-SIDED so a sensitivity regression fails in either direction.
#
# Reason for 0.60: the worst deviation of any corpus entry from the fitted curve
# is 0.432 (`aniso_free_dir_weak`, 1.762x above), so 0.60 holds the measured
# scatter with 1.39x of margin.
#
# WHAT IT BUYS, MEASURED RATHER THAN DERIVED, AND IT IS COARSER THAN THE ARITHMETIC
# SUGGESTS. A band of 0.60 stops catching at a ratio of 2.5x up and 0.40x down,
# but each entry's clean ratio already sits between 0.788 and 1.762, and an
# injected change MULTIPLIES that -- so the change has to clear the edge from
# wherever the entry already sits. Bracketed by injection on every corpus entry:
#
...
#
# So this assertion detects a 5x change in the gate's sensitivity and NOTHING
# FINER, and the derivation that said 2.5x ignored the scatter it sits on top of.
# That is far coarser than the fixed 1e-7 counter it replaces. It is also true at
# every slenderness, which the fixed counter was not: that counter was below the
# response at `L/r_min = 648` and the corpus contains entries past it.
# Set: 2026-09-06, F2
PATCH_TEST_SENSITIVITY_BAND: Final[float] = 0.60

# COUNTER-CASE, INJECTED (BG1): a multiplicative change in the gate's sensitivity,
# applied in both directions, that the band must catch on EVERY corpus entry.
# Reason for 5.0: it is the smallest of the bracketed factors caught in both
# directions -- 4x is caught upward and 1/4 is not, 3x and 1/3 are both missed.
# The bracket is in the band's own entry above, and it is why this is 5.0 rather
# than the 2.5 the band's arithmetic alone would give.
# Set: 2026-09-06, F2
PATCH_TEST_SENSITIVITY_BAND_COUNTER: Final[float] = 5.0
