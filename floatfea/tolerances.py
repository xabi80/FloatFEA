...
# derivation, 2026-09-05, the band was 577x wide and 5e-15 sat 26.8x above the
...
# rounds -- the clean side alone moved to 10.2x. What the value must satisfy is
# checked live and generated: `clean_worst_ratio` below one, and every
# `margin_<defect>` row for the asserted defects above it, in
...
# same. Both directions that exist point the same way, and two consecutive
# reviewer rounds measured it: the room went `2.37x`, `2.19x`, `2.18x`. The
# published room is what absorbs that while still catching every
# order-of-magnitude raise, and the demonstration that it catches one is a
# shipped test.
#
