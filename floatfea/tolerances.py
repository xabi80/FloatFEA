...
#
# Reason for 2.0: the measured worst over all thirteen pairs on the CANONICAL
# machine is `1.0` ULP of the channel's own amplitude, and most are `0.5`; on
# the machine the fixture was produced on it is `0.0`, every value exact. The
# same `1.0` is reported by every determinism leg, across six CPU models. `2.0`
# is twice the measured worst, which is the same rule
# `EXEMPT_RESPONSE_DRIFT_ULP` was set by, and it is the number Q8 already fixed
# for this class before any of it was measured.
