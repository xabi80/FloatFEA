...
# already wrong in the fourth digit when published (R176). At the values that
# produced this constant the ratio sat at `2.5e+07`, so this leaves about
# `2.4x`. The margin is eaten from BOTH sides -- a harder
...
# class is `DELTA_CALIBRATION_ULP`'s seeded histogram, whose observed maximum is
# 2 ULP. `4.0` is twice that maximum -- room for a platform whose last bit
